...
func (a *App) RegisterRoutes(r *gin.Engine) {
	...
	tokenManager := identity.NewTokenManager(
	...
	authRepo := identity.NewRepository(a.DB)
	authService := identity.NewService(authRepo, tokenManager)
	authHandler := identity.NewHandler(authService, a.Validator)
	authMW := identity.AuthMiddleware(tokenManager)
	identity.RegisterRoutes(api, authHandler, authMW)
	...
	bus.Subscribe(eventbus.EventTrackPlayed, analyticsEvents.OnTrackPlayed)
	bus.Subscribe(eventbus.EventTrackPlayed, historyEvents.OnTrackPlayed)
	bus.Subscribe(eventbus.EventTrackPlayed, recommendationEvents.OnTrackPlayed)

	bus.Subscribe(eventbus.EventPlaylistCreated, analyticsEvents.OnPlaylistCreated)
	bus.Subscribe(eventbus.EventPlaylistCreated, notificationEvents.OnPlaylistCreated)

	bus.Subscribe(eventbus.EventSubscriptionPurchased, analyticsEvents.OnSubscriptionPurchased)
	bus.Subscribe(eventbus.EventSubscriptionPurchased, notificationEvents.OnSubscriptionPurchased)

	bus.Subscribe(eventbus.EventUserRegistered, analyticsEvents.OnUserRegistered)
	bus.Subscribe(eventbus.EventUserRegistered, notificationEvents.OnUserRegistered)
}
