  ...
  <div class="mx-auto w-full max-w-7xl px-4 pt-6 pb-32 md:px-6 lg:px-8">
    ...
    >
      ...
      <p class="mt-4 max-w-2xl text-white/80">
        The most played tracks across the catalog right now.
      </p>
    </section>
    ...
    <section class="mt-10">
      ...
      >
        ...
        >
          <TrendingUp aria-hidden="true" class=""  />
        </div>
...
import { Music, TrendingUp } from 'lucide-vue-next'
...
onMounted(async () => {
  loading.value = true
  try {
    const response = await api.getPopular({ limit: 50 })
