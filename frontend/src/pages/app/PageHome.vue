<template>
  <div class="relative mx-auto w-full max-w-7xl px-4 pt-6 pb-36 md:px-6 lg:px-8">
    ...
    <div class="aurora-bg pointer-events-none fixed inset-0" aria-hidden="true">
      ...
      <div class="aurora-spot-1 top-20 left-1/3 bg-violet-500/10" style="animation-delay: -8s; width: 350px; height: 350px;" />
    ...
    <div class="relative z-10">
      ...
      <template v-else-if="!hasData">
        <section class="relative mt-8 overflow-hidden rounded-[2rem] border border-white/[0.06] bg-[#0C0C14] p-10 text-white shadow-2xl">
          <div class="absolute -top-20 -right-20 h-72 w-72 rounded-full bg-[#1db954]/20 blur-3xl" />
          <div class="absolute -bottom-20 -left-20 h-44 w-44 rounded-full bg-white/[0.03] blur-3xl" />

          <div class="relative z-10 max-w-3xl">
            <p class="text-[10px] font-bold tracking-[0.35em] text-white/30 uppercase">Welcome</p>
            <h1 class="mt-4 text-4xl leading-tight font-black md:text-6xl">
              Your music journey starts here.
            </h1>
            <p class="mt-5 max-w-xl text-sm leading-relaxed text-white/50">
              Stream tracks, follow artists, save albums, and build the perfect playlist. Everything you love in one place.
            </p>
            <div class="mt-8 flex flex-wrap gap-3">
              <RouterLink
                to="/search"
                class="inline-flex items-center gap-2 rounded-full bg-[#1db954] px-6 py-3 text-sm font-bold text-black transition hover:scale-105 hover:bg-[#1ed760]"
              >
              ...
              >
                <i aria-hidden="true" class="pi pi-compass" />
                Discover
              </RouterLink>
            </div>
          </div>
      ...
      <template v-else>
        ...
        <section v-if="popular.length" class="mt-12">
          <HomeSectionHeader title="Trending" eyebrow="Popular right now" see-all-route="/recommendations/popular" />
          <HomeCarousel>
            <HomeTrackCard
        ...
        <section v-if="forYou.length" class="mt-12">
          <HomeSectionHeader title="For You" eyebrow="Personalized picks" see-all-route="/recommendations/for-you" />
          <HomeCarousel>
            <HomeTrackCard
        ...
        <section v-if="albums.length" class="mt-12">
          ...
          <HomeCarousel>
            ...
            >
              <div class="relative aspect-square overflow-hidden rounded-xl bg-white/[0.06] shadow-lg ring-1 ring-white/10 transition-all duration-300 hover:shadow-[0_8px_32px_rgba(0,0,0,0.5)] hover:-translate-y-1 hover:ring-[#1db954]/30">
                <img
                ...
                <div class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition group-hover:opacity-100">
                  <div class="flex h-12 w-12 items-center justify-center rounded-full bg-[#1db954] text-black shadow-xl backdrop-blur-sm transition-transform group-hover:scale-110">
              ...
              <div class="space-y-0.5 px-1">
                ...
                <p v-if="album.artist_name" class="truncate text-xs text-white/40">{{ album.artist_name }}</p>
                <p class="text-xs text-white/30">{{ album.release_date?.slice(0, 4) || '' }}{{ album.track_count ? ` \u2022 ${album.track_count} tracks` : '' }}</p>
              </div>
            </RouterLink>
          </HomeCarousel>
        </section>
        ...
        <section class="mt-12">
          <div class="grid gap-4 md:grid-cols-3">
            <RouterLink
              to="/recommendations"
              class="group rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 transition hover:-translate-y-0.5 hover:border-white/[0.12] hover:bg-white/[0.04]"
            >
              <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-white/[0.06] text-white/50">
                <i aria-hidden="true" class="pi pi-star text-xl" />
              </div>
              <h3 class="mt-4 text-sm font-bold text-white">Recommendations</h3>
              <p class="mt-1 text-sm text-white/40">Discover popular, recent, and personalized tracks.</p>
            </RouterLink>
            <RouterLink
              to="/discover"
              class="group rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 transition hover:-translate-y-0.5 hover:border-white/[0.12] hover:bg-white/[0.04]"
