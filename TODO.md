...
### 5. API contract mismatch — camelCase vs snake_case
...
### 6. No durable media asset table
...
### 8. Player queue persists full objects in localStorage
...
### 9. Shuffle has no history stack
...
### 10. No retry on audio playback error
...
### 11. `resetTracks()` called but not implemented
...
### 12. JWT secrets have insecure defaults
...
### 13. Upload lacks duration extraction
...
### 14. Media serving is not range-aware
...
### 16. Frontend organized by type, not domain
...
### 17. Backend DTOs double as domain models
...
### 18. No middleware consolidation
...
### 23. `formatDuration` treats 0 as falsy
...
### 27. `RegisterTrack` reads raw body after `ShouldBindJSON` failure

Consumes the request body, making error logs misleading.

**File:** `internal/modules/catalog/handler.go`

### 28. Auth store is not strict-safe
...
### 34. `RequireRole` is exact-match only
...
### 35. Album upsert doesn't fall back to embedded cover
